...
int GetPvLine(const int depth, S_BOARD *pos) {
	...
	while(move != NOMOVE && count < depth) {
		...
		if(MoveExists(pos, move)) {
			MakeMove(pos, move);
			pos->PvArray[count++] = move; //need to run this in order to build pv line
		} else {
			break;
		}
...
void InitHashTable(S_HASHTABLE *table, const int size) {
	int HashSize = 0x100000 * size;
...
int ProbeHashEntry(S_BOARD *pos, int *move, int *score, int alpha, int beta, int depth) {
	...
	if(pos->HashTable->pTable[index].posKey == pos->posKey) {
		*move = pos->HashTable->pTable[index].move;
		//printf("debug1\n");
		//printf("move:%s score:%d depth:%d\n", PrMove(pos->HashTable->pTable[index].move), pos->HashTable->pTable[index].score, pos->HashTable->pTable[index].depth);
		//printf("move:%s depth:%d\n", PrMove(*move), depth);
